...
public class AuthService {
    ...
    public AuthTokensResponse register(RegisterRequest request) {
        if (request.device() == null) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Device is required");
        }
        ...
        Device device = deviceService.upsert(savedUser.getId(), request.device());

        String access = jwtTokenService.issueAccessToken(savedUser.getId(), savedUser.getEmail());
        String refresh = refreshTokenService.issue(savedUser.getId(), device.getId());
    ...
    public AuthTokensResponse login(LoginRequest request) {
        ...
        Device device = deviceService.upsert(user.getId(), request.device());

        String access = jwtTokenService.issueAccessToken(user.getId(), user.getEmail());
        String refresh = refreshTokenService.issue(user.getId(), device.getId());
        return new AuthTokensResponse(access, refresh, jwtTokenService.accessTokenExpiresInSeconds());
    }
    ...
    public AuthTokensResponse refresh(RefreshRequest request) {
        ...
        String access = jwtTokenService.issueAccessToken(user.getId(), user.getEmail());
        String refresh = refreshTokenService.issue(user.getId(), existing.getDeviceId());
        return new AuthTokensResponse(access, refresh, jwtTokenService.accessTokenExpiresInSeconds());
    }
